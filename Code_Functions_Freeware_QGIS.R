###Functions for QGIS method###
#Run all functions so they are available in the environment for use in 'Code_Freeware_QGIS.R'
...
bird_model_allvars <- function(data, num.trees = 1000, leaf.size = 5, mtry = NULL) {
  fm <- ranger(formula = birdornot ~ meanB0 + meanB1 + meanB2 + varB0 + varB1+ varB2 + nbPixels,
               data = data,
...
species_model_allvars <- function(data, num.trees = 1000, leaf.size = 1, mtry = NULL) {
  fm <- ranger(formula = bird ~  meanB0 + meanB1 + meanB2 + varB0 + varB1+ varB2 + nbPixels,
               data = data,
               num.trees = num.trees, min.node.size = leaf.size, mtry = mtry,
               importance = "impurity", write.forest = T)
  print(fm)
